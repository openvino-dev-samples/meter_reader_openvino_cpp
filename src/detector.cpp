...
bool Detector::init(string model_path, double threshold)
{
    ...
    this->infer_request = detect_model.create_infer_request();
    return true;
}
...
bool Detector::run(Mat &src_img, vector<Rect> &detected_objects)
{
    ...
    ov::Tensor input_tensor0 = this->infer_request.get_tensor("im_shape");
    ov::Tensor input_tensor1 = this->infer_request.get_tensor("image");
    ov::Tensor input_tensor2 = this->infer_request.get_tensor("scale_factor");
    // nhwc -> nchw
    ...
    for (int num = 0; num < total_num; num++)
    {
        ...
        if (box_prob > this->threshold && box_prob <= 1)
        {
